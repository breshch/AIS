...
{
	public class ProfitViewModel : ViewModelGlobal
	{
		...
		private Tuple<double, double> GetRealAndOvertimeSalary(int year, int month)
		{
			...
			var workers = BC.GetDirectoryWorkersByTypeOfPost(year, month, false);
			var currentPosts = BC.GetCurrentPosts(lastDate);
			var infoDatesWorkers = BC.GetInfoDatesByMonth(year, month);
			var salaries = BC.GetDirectoryPostSalariesByMonth(year, month);
		...
		private Tuple<double, double> GetMinskAndOvertimeSalary(int year, int month)
		{
			...
			var overTimes = BC.GetInfoOverTimes(year, month);
			var weekEndsInMonth = BC.GetHolidaysByMonth(year, month);
