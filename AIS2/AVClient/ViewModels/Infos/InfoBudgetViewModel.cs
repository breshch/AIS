...
using Shared.Enums;
using CashType = AVClient.AVServiceReference.CashType;
...
namespace AVClient.ViewModels.Infos
{
    public class InfoBudgetViewModel : ViewModelGlobal
    {
        ...
        {
            InfoSafesCash = new ObservableCollection<InfoSafe>(BC.GetInfoSafesByCashType(CashType.Наличка, SelectedInfoCashFromDate, SelectedInfoCashToDate));
            RefreshAllSafeData();
        }

        private void RefreshInfoSafesCard()
        {
			InfoSafesCard = new ObservableCollection<InfoSafe>(BC.GetInfoSafesByCashType(CashType.Карточка, SelectedInfoCardFromDate, SelectedInfoCardToDate));
            RefreshAllSafeData();
        }
