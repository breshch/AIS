...
using AVClient.ViewModels.Infos;
...
using DescriptionDay = AVClient.AVServiceReference.DescriptionDay;
using MenuItem = System.Windows.Controls.MenuItem;
using MessageBox = System.Windows.MessageBox;
using TextBox = System.Windows.Controls.TextBox;

namespace AVClient.Views
{
    public partial class MonthTimeSheetView : Window
    {
        ...
        private bool _isEnableTimerAbsentDates;
        ...
        private void SetScreenSize()
        {
            ...
            Left = firstWorkingArea.Width / 2 - (Width / 2);
            MaxHeight = firstWorkingArea.Height - 100;
        }
        ...
        {
            var column = FindName("MonthTimeSheet" + privilege) as DataGridTextColumn;
        ...
        {
            _privileges = Privileges.UserPrivileges;
            ...
            Task.Factory.StartNew(() =>
                {
                    ...
                    _weekends = _bc.GetHolidaysByMonth(_currentYear, _currentMonth).ToList();

                    var workers = _bc.GetDirectoryWorkersMonthTimeSheet(_currentYear, _currentMonth).ToList();

                    var infoDates = _bc.GetInfoDatesByMonth(_currentYear, _currentMonth);

                    var infoMonthes = _bc.GetInfoMonthes(_currentYear, _currentMonth).ToList();
                    int indexWorker = 0;

                    var workerWarehouses = workers.Where(w => !w.IsDeadSpirit && _bc.GetDirectoryTypeOfPost(w.Id, lastDateInMonth) == TypeOfPost.Warehouse).OrderBy(w => w.LastName).ToList();
                    ...
                    {
                        var workerOffices = workers.Where(w => !w.IsDeadSpirit && _bc.GetDirectoryTypeOfPost(w.Id, lastDateInMonth) == TypeOfPost.Office).ToList();
                ...
                }).ContinueWith(task =>
                    {
                        Dispatcher.BeginInvoke(new Action(() =>
                        {
                            ...
                            for (int i = 0; i < lastDateInMonth.Day; i++)
                            {
                                ...
                                {
                                    Header = (i + 1) + Environment.NewLine + new DateTime(_currentYear, _currentMonth, i + 1).ToString("ddd"),
                                    Binding = new Binding
                                    {
            ...
            DateTime lastDateInMonth, DateTime firstDateInMonth, InfoDate[] infoDates, List<InfoMonth> infoMonthes)
        {
            ...
            foreach (var worker in workers)
            {
                ...
                var currentPosts = _bc.GetCurrentPostsByWorker(worker.Id, _currentYear, _currentMonth, _countLastDaysInMonth).ToList();
            ...
            if (indexColumn > COUNT_COLUMNS_BEFORE_DAYS && indexColumn <= _countLastDaysInMonth + COUNT_COLUMNS_BEFORE_DAYS)
            {
                ...
                if (double.TryParse(value, out resultValue))
                {
                    if ((resultValue > 8 || isWeekend))
                    {
                        ...
                        _bc.EditInfoOverTimeByDate(date, maxOverTime);
        ...
        private void MenuReportCosts_Click(object sender, RoutedEventArgs e)
        {
            HelperMethods.ShowView(new MonthReportViewModel(
               ...
               {
                   var directoryRCs = _bc.GetDirectoryRCsMonthIncoming(SelectedYear, SelectedMonth).ToList();

                   foreach (var rc in directoryRCs)
                   {
                       CashReports.IncomingRC(rc, SelectedYear, SelectedMonth);
                   }

                   CashReports.Incoming26(SelectedYear, SelectedMonth);

                   CashReports.Expense26(SelectedYear, SelectedMonth);

                   CashReports.ExpenseRCs(SelectedYear, SelectedMonth);

                   CashReports.ExpensePAM16(SelectedYear, SelectedMonth);
               },
               () => _bc.GetInfoCostYears().OrderBy(y => y).ToList(),
               year => _bc.GetInfoCostMonthes(year).OrderBy(m => m).ToList()
               ), new MonthReportView());
        }
        ...
        private void MenuReportSalaryPrint_Click(object sender, RoutedEventArgs e)
        {
            HelperMethods.ShowView(new MonthReportViewModel(
               ...
               () => _bc.GetYears().OrderBy(y => y).ToList(),
               year => _bc.GetMonthes(year).OrderBy(m => m).ToList()
               ), new MonthReportView());
        }
