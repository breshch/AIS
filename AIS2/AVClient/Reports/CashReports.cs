...
namespace AVClient.Reports
{
    public static class CashReports
    {
        ...
        public static void IncomingRC(DirectoryRC rc, int year, int month)
        {
            ...
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    ep => IncomingRCReport(ep, year, month, rc)
                });
        }

        public static void Incoming26(int year, int month)
        {
            string path = Path.Combine(PATH_DIRECTORY_COST_REPORTS, PATH_INCOMING_REPORT + " " + OFFICE_NAME + " " + month + "." + year + ".xlsx");
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    ep => Incoming26Report(ep, year, month)
                });
        }

        public static void ExpenseRCs(int year, int month)
        {
            string path = Path.Combine(PATH_DIRECTORY_COST_REPORTS, PATH_EXPENSES_REPORT + " " + month + "." + year + ".xlsx");
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    ep => ExpenseRCsReport(ep, year, month)
                });
        }

        public static void Expense26(int year, int month)
        {
            string path = Path.Combine(PATH_DIRECTORY_COST_REPORTS, PATH_EXPENSE_REPORT + " " + OFFICE_NAME + " " + month + "." + year + ".xlsx");
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    ep => Expense26Report(ep, year, month)
                });
        }

        public static void ExpensePAM16(int year, int month)
        {
            string path = Path.Combine(PATH_DIRECTORY_COST_REPORTS, PATH_EXPENSE_REPORT + " " + RC_PAM16 + " " + month + "." + year + ".xlsx");
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    ep => ExpensePAM16Report(ep, year, month)
                });
        }

        private static void IncomingRCReport(ExcelPackage ep, int year, int month, DirectoryRC rc)
        {
        ...
        private static void Incoming26Report(ExcelPackage ep, int year, int month)
        {
            ...
            var totalSummsCurrency = bc.GetInfoCostsIncomingTotalSummsCurrency(year, month, "26А", true, null).ToList();
        ...
        private static void Expense26Report(ExcelPackage ep, int year, int month)
        {
        ...
        private static void ExpensePAM16Report(ExcelPackage ep, int year, int month)
        {
        ...
        private static void ExpenseRCsReport(ExcelPackage ep, int year, int month)
        {
