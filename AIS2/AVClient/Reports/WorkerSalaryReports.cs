...
namespace AVClient.Reports
{
	public static class WorkerSalaryReports
	{
		...
		public static void ComplitedMonthCashReportMinsk(DateTime from, DateTime to)
		{
			Helpers.CompletedReport(PATH_CASH_REPORT, new List<Action<ExcelPackage>>
                {
	                ep =>
	                {
		                ...
		                {
			                MonthCashReportMinsk(ep, date.Year, date.Month);
		                }
	                }
                });
		}
		...
		private static void MonthCashReportMinsk(ExcelPackage ep, int year, int month)
		{
			...
			int indexRow = 2;
			while (sheet.Cells[indexRow, 2].Value != null)
			{
				indexRow++;
			}

			Helpers.CreateCell(sheet, indexRow, 2, month + "'" + year, Color.DarkCyan, 12, true, ExcelHorizontalAlignment.Center, ExcelBorderStyle.Thick);
		...
		private static void OverTimeReportMinsk(ExcelPackage ep, int year, int month)
		{
			...
			var workers = bc.GetDirectoryWorkersByTypeOfPost(year, month, false).ToList();
			...
			var weekEndsInMonth = bc.GetHolidaysByMonth(year, month).ToList();
			...
			foreach (var overTime in overTimes)
			{
				...
				for (int i = 0; i < countRCs; i++)
				{
					...
					foreach (var worker in workers)
					{
						...
						if (infoDate != null)
						{
							var overTimeHours = bc.IsOverTime(infoDate, weekEndsInMonth.ToArray());
		...
		private static void SalaryReportMinsk(ExcelPackage ep, int year, int month)
		{
			...
			var warehouseWorkers = bc.GetDirectoryWorkersByTypeOfPost(year, month, false);

			var overTimes = bc.GetInfoOverTimes(year, month).ToList();
			var weekEndsInMonth = bc.GetHolidaysByMonth(year, month).ToList();
			var workerSumms = new List<WorkerSummForReport>();

			var currentRCs = bc.GetCurrentRCs(overTimes.Select(o => o.Id).ToArray());
			...
			foreach (var overTime in overTimes)
			{
				var overTimeRCs = currentRCs.Where(r => r.InfoOverTimeId == overTime.Id).ToArray();
			...
			var officeWorkers = bc.GetDirectoryWorkersByTypeOfPost(year, month, true).ToList();
		...
		private static void SalaryReportWorkers(ExcelPackage ep, int year, int month)
		{
			...
			foreach (var worker in workers)
			{
				...
				var infoDates = bc.GetInfoDatesByWorker(worker.Id, year, month);
				...
				for (int i = 0; i < workerPostReportSalaries.Count - 1; i++)
				{
					...
					downLeftBodySalaryInHour.Value = Math.Round((workerPostReportSalaries[i].AdminWorkerSalary / countWorkDays / 8), 2) + " р.";
					...
					downLeftBodySummOfCharge.Value = Math.Round((workerPostReportSalaries[i].AdminWorkerSalary / countWorkDays / 8) * workerPostReportSalaries[i].CountWorkHours, 2) + " р.";
				...
				downLeftBodyLastSalaryInHour.Value = Math.Round((workerPostReportSalaries[workerPostReportSalaries.Count - 1].AdminWorkerSalary / countWorkDays / 8), 2) + " р.";
				...
				downLeftBodyLastSummOfCharge.Value = Math.Round((workerPostReportSalaries[workerPostReportSalaries.Count - 1].AdminWorkerSalary / countWorkDays / 8) *
				...
				if (workerPostReportSalaries.Any(w => w.CountWorkOverTimeHours > 0))
				{
					...
					for (int i = 0; i < workerPostReportSalaries.Count - 1; i++)
					{
						...
						downLeftBodySalaryInHour.Merge = true;
						downLeftBodySalaryInHour.Style.Border.BorderAround(ExcelBorderStyle.Thin);
						downLeftBodySalaryInHour.Value = Math.Round(((workerPostReportSalaries[i].AdminWorkerSalary / countWorkDays / 8) * 2), 2) + " р.";

						ExcelRange downLeftBodySummOfCharge = sheet.Cells[countRow, 18, countRow, 20];
						downLeftBodySummOfCharge.Merge = true;
						downLeftBodySummOfCharge.Style.Border.BorderAround(ExcelBorderStyle.Thin);
						downLeftBodySummOfCharge.Value = Math.Round(((workerPostReportSalaries[i].AdminWorkerSalary / countWorkDays / 8) *
					...
					downLeftBodyOvertimeLastSalaryInHour.Value = Math.Round(((workerPostReportSalaries[workerPostReportSalaries.Count - 1].AdminWorkerSalary / countWorkDays / 8) * 2), 2) + " р.";
					...
					downLeftBodyOvertimeLastSummOfCharge.Value = Math.Round(((workerPostReportSalaries[workerPostReportSalaries.Count - 1].AdminWorkerSalary / countWorkDays / 8) *
				...
				{
					allSalary += Math.Round((workerPostReportSalary.AdminWorkerSalary / countWorkDays / 8) * workerPostReportSalary.CountWorkHours, 2);
					allOverTimeSalary += Math.Round(((workerPostReportSalary.AdminWorkerSalary / countWorkDays / 8) * workerPostReportSalary.CountWorkOverTimeHours) * 2, 2);
				}
