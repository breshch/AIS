...
namespace AIS_Enterprise_AV.Reports
{
    public static class CarsReports
    {
        ...
        public static void Cars(DateTime from, DateTime to)
        {
            ...
            Helpers.CompletedReport(path, new List<Action<ExcelPackage>>
                {
                    (ep) =>
                    {
                        ...
                        {
                            CarsReport(ep, date.Year, date.Month);    
                        }
                    }
                });
        }

        private static void CarsReport(ExcelPackage ep, int year, int month)
        {
	        ...
	        using (var bc = new BusinessContext())
	        {
		        ...
		        var costsGroups = costs.GroupBy(c => c.GroupId).Select(g => new
		        {
			        Summ = g.Sum(c => c.Summ),
			        TransportCompany =
				        g.First().DirectoryTransportCompany != null ? g.First().DirectoryTransportCompany.Name : "",
			        Date = g.First().Date
		        });
